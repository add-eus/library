...
watch(searchDebounced, () => {
    if (searchDebounced.value && searchDebounced.value.length > 0)
        limit.value = 10;
    else
        limit.value = 0;
});
...
<template>
    <div
        ref="dropdownElement"
        class="dropdown is-modern is-spaced"
    >
        <slot :search="updateSearch"></slot>

        <div class="dropdown-menu" role="menu">
            <div class="dropdown-content">
                <VDropdownItem v-for="item in items" :key="item.$getID()" @click="select(item)">{{ item.toString() }}</VDropdownItem>
            </div>
        </div>
    </div>

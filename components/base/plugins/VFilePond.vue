...
import { onMounted, onUnmounted, ref, defineComponent, h, watch } from "vue";
            ...
            if (inputElement.value && FilePond.supported()) {
                ...
                const initCallback = () => {
                    for (const eventName of eventNames) {
                        ...
                        if (event) {
                            pond.value.on(event, (...eventArgs) => {
                                var files = pond.value ? pond.value.getFiles() : [];
                                emit("input", files);

                                emit(eventName, eventArgs[1]);
                            });
                        }
                    }
                };

                pond.value.onOnce("init", initCallback);

                for (const prop in _OptionTypes) {
                    // don't add events to the watch
                    if (/^on/.test(prop)) {
                        continue;
                    }

...
export interface VPhotoSwipeEmits {
    (e: "load"): void;
}
...
const emit = defineEmits<VPhotoSwipeEmits>();
...
let itemElements = ref(props.items);

async function fetchStorage() {
    if (props.storageItems && props.storageItems.length > 0) {
        itemElements.value = await Promise.all(
            props.storageItems.map(async (item) => {
                const refFile = refStorage(storage, item);
                const src = await getDownloadURL(refFile);
                const img = new Image();
...
const resetAngle = () => {
    if (pswpElement.value) {
        ...
        pswpElement.value
            .querySelectorAll(".pswp__img")
            .forEach(
                (i) => ((i as HTMLElement).style.transform = `rotate(${angle.value}deg)`)
            );
    }
};
</script>
...
<template>
    ...
    >
        <figure
            v-for="(item, index) in itemElements"

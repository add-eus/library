...
export function useCollection<T extends typeof Entity>(
  ...
  if (
    options.wheres !== undefined &&
    options.compositeConstraint !== undefined
  ) {
...
export function useDoc<T extends typeof Entity>(
  ...
  console.log("doc(collection(firestore, collectionName), id);");
...
export function newDoc<T extends typeof Entity>(
  collectionModel: T
): InstanceType<T> {
...
export async function findDoc<T extends typeof Entity>(
  ...
  const search: string = isRef(options.search)
    ? options.search.value
    : options.search;

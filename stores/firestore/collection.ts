...
export interface CollectionOptions<T> {
  ...
  backlistFields?: Partial<
    Omit<
      Record<keyof NonFunctionProperties<T>, boolean>,
  ...
  return function (target: any, propertyKey?: string) {
    ...
    // On property
      ...
      const onCollectionInitialize = () => {
        ...
        const blacklistedProperties = Object.entries(
          options.backlistFields ?? {}
        )
          .filter(([, value]) => value)
          .map(([key]) => key);
...
export class SubCollection<T extends Entity> {
  ...
  setOptions(
    options?: Omit<UseCollectionOption, "path" | "blacklistedProperties">
  ) {
    if (!this.initialized)
...
export const updatePropertyCollection = async (
  ...
  const addPromises = toAdd.map(async (entity) => {
    ...
    if (
      entity.$getMetadata().reference?.path === `${collectionRef.path}/${id}`
    ) {
      return;
    }
